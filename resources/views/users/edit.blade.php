            ...
            <form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-3">
                                <label>Asignatura Principal (solo para profesores)</label>
                                <select name="main_course_id" class="form-control">
                                    <option value="">Ninguna</option>
                                    @foreach($courses as $course)
                                        <option value="{{ $course->id }}" {{ old('main_course_id', $user->main_course_id) == $course->id ? 'selected' : '' }}>{{ $course->name }}</option>
                                    @endforeach
                                </select>
                                @error('main_course_id') <span class="text-danger small">{{ $message }}</span> @enderror
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group mb-3">
